...
namespace tcp_interface {
    ...
    TcpInterface::TcpInterface(const rclcpp::NodeOptions &options)
        ...
        srv_ = this->create_service<tcp_socket_msg::srv::TcpSocketICtrl>(
                "tcp_register",
    ...
    void TcpInterface::_subscriber_callback(const tcp_socket_msg::msg::TcpSocket msg) {
        if (this->_get_current_state().id() != lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE) {
            return;
        }
    ...
    LNI::CallbackReturn TcpInterface::on_configure(const rclcpp_lifecycle::State &state) {
        ...
        _subscription = this->create_subscription<tcp_socket_msg::msg::TcpSocket>(
                "tcp_connection",
                _qos,
    ...
    void TcpInterface::handleService_(const std::shared_ptr<tcp_socket_msg::srv::TcpSocketICtrl::Request> &request,
